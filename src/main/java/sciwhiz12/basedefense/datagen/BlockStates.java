...
public class BlockStates extends BlockStateProvider {
    ...
    protected void registerStatesAndModels() {
        ...
        // );
        //
        // VariantBlockStateBuilder bld =
        // getVariantBuilder(ModBlocks.LOCK_DOOR_BLOCK.get());
        //
        // registerParts(bld, DoubleBlockHalf.UPPER, createTriFunc(door_top_rh,
        // door_top));
        // registerParts(bld, DoubleBlockHalf.LOWER, createTriFunc(door_bottom_rh,
        // door_bottom));
    }
    //
    // private static BiFunction<DoorHingeSide, Boolean, ModelFile>
    // createTriFunc(ModelFile xorTrue,
    // ModelFile xorFalse) {
    // return (hinge, open) -> {
    ...
    // }
    //
    // private void registerParts(VariantBlockStateBuilder bld, DoubleBlockHalf
    // half,
    ...
    // bld.partialState().with(PadlockedDoorBlock.FACING, dir).with(
    //// PadlockedDoorBlock.HALF, half
    //// ).with(PadlockedDoorBlock.HINGE, hinge).with(PadlockedDoorBlock.OPEN,
    // open);
    //// ConfiguredModel model =
    // ConfiguredModel.builder().modelFile(modelFile).rotationY(
    ...
    // private void setupDoor(BlockModelBuilder builder, String texture, Direction
    // dir,
    // boolean rightHinge) {
    // builder.ao(false).texture("particle", texture);
    // ModelBuilder<?>.ElementBuilder el = builder.element().from(0f, 0f, 0f).to(3f,
    // 16f, 16f);
    // el.face(dir).uvs(13f, 0f, 16, 16f).texture("#bottom").cullface(dir);
    // setupAndEndFaces(el, texture, rightHinge);
    // }
    //
    // private void setupAndEndFaces(ModelBuilder<?>.ElementBuilder el, String
    // texture,
    // boolean rightHinge) {
    // float d = rightHinge ? 16F : 0F;
    // el.face(Direction.SOUTH).texture(texture).uvs(0f, 0f, 3f,
    // 16f).cullface(Direction.SOUTH);
    // el.face(Direction.NORTH).texture(texture).uvs(3f, 0f, 0f,
    // 16f).cullface(Direction.NORTH);
    // el.face(Direction.WEST).texture(texture).uvs(16f - d, 0f, d,
    // 16f).cullface(Direction.WEST);
